...
public class DataExportingServiceImpl implements DataExportingService {
	...
	public AthleteAttDao getAthleteAttDao() {
		return athleteAttDao;
	}

	public void setAthleteAttDao(AthleteAttDao athleteAttDao) {
		this.athleteAttDao = athleteAttDao;
	}
		...
		if(athleteViews != null){
			...
			try {
				...
				for(int i=6;i<athleteViewSize+6;i++){
					...
					sheet.addCell(new Label(3, i,position));
					sheet.addCell(new Label(4, i,athleteView.getAthleteHeight()+""));
					sheet.addCell(new Label(5, i,athleteView.getAthleteWeight()+""));
					sheet.addCell(new Label(6, i, athleteView.getIdentifiedId()+""));
				}
				//写进文档
				wwb.write();
				//					// 关闭Excel工作簿对象
...
//			String titleString = "南京市" + calendar.get(Calendar.YEAR) + "年校园足球联赛报名表";
			String titleString = leagueName + "报名表";
...
//			doc.add(leaderContext3);
	...
	public List<CardSchoolView> exportCard_School() {
		List<YsmsSchool> schools = schoolDao.findByNameAndCategory(null, null);
		List<CardSchoolView> exports = new ArrayList<CardSchoolView>();
		for(int i=0;i<schools.size();i++){
			YsmsSchool school = schools.get(i);
			CardSchoolView csv = new CardSchoolView();
			csv.setSchoolId(school.getSchoolId());
			csv.setSchoolName(school.getSchoolName());
			exports.add(csv);
		}
		return exports;
	}
	...
	public List<CardAthleteView> exportCard_Athlete(int schoolId) {
		List<YsmsAthlete> athletes = athleteDao.findByFuzzyQuery(schoolId, null, null, null, null, null, null);
		...
		for(int i=0;i<athletes.size();i++){
			...
			cav.setSchool(schoolDao.findById(schoolId).getSchoolName());
	...
	public List<CardCoachView> exportCard_Coach(int schoolId) {
		List<YsmsCoach> coaches = coachDao.findByFuzzyQuery(null, schoolId, null, null, null);
		List<CardCoachView> exports = new ArrayList<CardCoachView>();
		for(int i=0;i<coaches.size();i++){
			YsmsCoach coach = coaches.get(i);
			CardCoachView ccv = new CardCoachView();
			String certificate = "";
			if(coach.getSchoolcoachFlag()==true&&coach.getCoachLevel()>0&&coach.getCoachLevel()<5){
				certificate = "校园足球证书,";
				switch(coach.getCoachLevel()){
				case 1:
					certificate += "中国足协D级教练员证书";
					break;
				case 2:
					certificate += "中国足协C级教练员证书";
					break;
				case 3:
					certificate += "中国足协B级教练员证书";
					break;
				case 4:
					certificate += "中国足协A级教练员证书";
					break;
				}
			}
			else if(coach.getSchoolcoachFlag()==true){
				certificate = "校园足球证书";
			}
			else if(coach.getCoachLevel()>0&&coach.getCoachLevel()<5){
				switch(coach.getCoachLevel()){
				case 1:
					certificate += "中国足协D级教练员证书";
					break;
			...
			ccv.setCertificate(certificate);
			ccv.setCoachId(coach.getCoachId());
			ccv.setGender(coach.getIdentifiedGender()==0?"女":"男");
			ccv.setIdentifiedNum(coach.getIdentifiedId());
			ccv.setName(coach.getIdentifiedName());
			ccv.setNation(coach.getIdentifiedNationality());
			ccv.setPhone(coach.getCoachMobile());
			ccv.setPhotoBase64(coachAttDao.findByCoachId(coach.getCoachId()).getAttName());
			ccv.setSchool(schoolDao.findById(schoolId).getSchoolName());
			exports.add(ccv);
		}
		return exports;
	}
